...
\begin{description}
  \item[Preparation of Datasets:] One of the most general questions when it comes to these datasets is how to organize and prepare the dataset. Where should the data come from? Data can be collected by organizations, by the instructor, by the students (e.g., using data in their own lives), or any of a number of different sources. That data can be based on a simulation or predicted model (e.g., fantasy football), or observations of a real phenomenon (e.g., weather reports)? I will analyze these different sources, the affordances and costs associated with them, and develop conclusions on the best way to leverage them. This will include guides on how to find and develop datasets.
	...
	\item[Data Variety] Data can often be 
\end{description}

\subsubsection{Data Discoverability}

\textbf{How can we minimize the cognitive load required for students to start using a given dataset?}
...
\subsection{Comparison of Contexts}
...
\subsubsection{Instructor Guides}

\subsubsection{Developer Guides}


One of the biggest question lurking in my research is: how strong a preference do students have towards different contexts?
It is extremely challenging to ask this question, because students cannot be exposed to all the different kinds of contexts.
Furthermore, different learners respond to different contexts in their own way, and it may not be possible to generalize any results found.
By studying large user populations (possibly in concert with other universities), the latter can be mitigated.
To mitigate the former, it is possible to expose students to representative samples of other contexts. 
Some of this requires immense new technology, e.g., integrating recent work with Pixly \cite{pixly} that adds media computation into Blockly environments could enable students to try media computation in the same course.
Although limited, this at least will start to answer this research question.

\subsubsection{More Libraries}
